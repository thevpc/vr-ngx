...
export class VrHttp {
  ...
  constructor(private http: HttpClient, private model: VrSharedState) {
  ...
  public authenticate(username: any, password: any): Observable<CurrentUser> {
    ...
    if (username == null || username == '' || password == null || password == '') {
      return this.throwSecurityException('Empty UserName or password');
    }
    const apiURL = `${this.model.apiRoot}/login?app=ng&login=${username}&password=${password}`;
    // alert('authenticate : ' + apiURL);
    return this.http.get(apiURL, { observe: 'response' }).map(res => {
    ...
    if (this.isValidSession()) {
      ...
      return this.authenticatedWScript(script).catch(err => {
        ...
        if (this.model.credentials == null) {
          return this.throwSecurityException('Invalid Session, Credentials empty');
        }
        return this
      ...
      if (!this.model.credentials) {
        return this.throwSecurityException('Invalid Session, Credentials empty2');
      }
      return this
        .authenticate(this.model.credentials.name, this.model.credentials.pwd)
        .concat(this.authenticatedWScript(script));
  ...
  public authenticatedCall(apiURL: string): Observable<any> {
    ...
    if (sessionId == null || sessionId == '') {
      ...
      return this.throwSecurityException('Invalid Empty Session');
    }
    // alert('authenticatedCall ' + apiURL+" ## "+ sessionId);
    if(apiURL.indexOf("?")){
      apiURL+="&jsessionid="+sessionId;
    }else{
      apiURL+="?jsessionid="+sessionId;
    }
  ...
  private throwSecurityException(message:string): Observable<any> {
    return this.throwMsg('FastSecurityException', 'Not Connected...'+message);
  }
